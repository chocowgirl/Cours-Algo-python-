...
affiche_tableau(reject_tableau1)

###### version sans index #########
#ceci est très adapté à python#
"""
def affiche_tableau(tableau):
    for line in tableau: #traite des lignes comme entiers
        txt = ""
        for element in line:
            txt += str(tableau[line][col]) + " "  <-----Faut vérifier contre ex de prof (pe faute de tapage)
            if element < 10:
                txt = txt + str(element) + "  "
